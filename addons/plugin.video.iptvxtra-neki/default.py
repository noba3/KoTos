...
import resources.requests as requests
import resources
__plugin__ = resources.Plugin()
...
def _request(url_path, params={}):
    language = resources.getLanguageId('de-DE')
...
def _listPosts(json_posts):
    ...
    __plugin__.addSortMethod(resources.plugin.__SORT_METHOD_VIDEO_TITLE__)
    __plugin__.addSortMethod(resources.plugin.__SORT_METHOD_VIDEO_RATING__)
    __plugin__.addSortMethod(resources.plugin.__SORT_METHOD_VIDEO_YEAR__)
    __plugin__.addSortMethod(resources.plugin.__SORT_METHOD_DATEADDED__)
    
    for post in json_posts:
        id = post.get('id', None)
        name = post.get('title', None)
        thumbnailImage = post.get('thumbnail', '')
        plot = resources.stripHtmlFromText(post.get('content', ''))
...
def play(id):
    ...
    plx = {'loc': streamer_url+id, 'service':'netzkino'}
    url = requests.get("http://www.iptvxtra.net/xbmc/_scripts/setlocation.php", params=plx)
    url = url.text.strip()
...
def search():
    ...
    keyboard = resources.Keyboard(__plugin__.localize(30000))
    if keyboard.doModal():
        success = True
...
action = resources.getParam('action')
id = resources.getParam('id')
