...
def Basla():
        ...
        if ("okmember" in membership and settings.getSetting( 'favori' ) == "false" ):
                addDir('[COLOR orange][B][COLOR blue]* [/COLOR]'+ 'Favorilerim' +'[/B][/COLOR]'+'[COLOR blue]* [/COLOR]',mrl + "&tip=favori&url="+root()+"data/"+ hashlib.md5(settings.getSetting( "mail" )).hexdigest()+".xml",2,root()+'resimler/favori.png')
...
def VKoynat(url):
        ...
        sorgu = url.split('?', 1)[-1]
        sorgu = parse_qs(sorgu)
        url = 'http://api.vk.com/method/video.getEmbed?oid=%s&video_id=%s&embed_hash=%s' % (sorgu['oid'][0], sorgu['id'][0], sorgu['hash'][0])
        request = urllib2.Request(url, None, {'User-agent': 'Mozilla/5.0 (Windows; U; Windows NT 5.1; en-GB; rv:1.9.0.3) Gecko/2008092417 Firefox/3.0.3','Connection': 'Close'})
        page = urllib2.urlopen(request).read()
        er = re.findall('error_msg":"([^"]+)', page)
        if er:
                url = 'http://vk.com/al_video.php?act=show_inline&al=1&video=%s_%s' % (sorgu['oid'][0], sorgu['id'][0])
                request = urllib2.Request(url, None, {'User-agent': 'Mozilla/5.0 (Windows; U; Windows NT 5.1; en-GB; rv:1.9.0.3) Gecko/2008092417 Firefox/3.0.3','Connection': 'Close'})
                page = urllib2.urlopen(request).read()
                page = re.sub('\\\\', '', page)
        videolist = re.findall('"url[^"]+":"(http:[^"]+)', page)
        qualitylist = re.findall('"url([^"]+)":"http:[^"]+', page)
        dialog = xbmcgui.Dialog()
        ret = dialog.select('kalite secin...',qualitylist)
        return videolist[ret]
def Picasaweb(url):
        idd = re.findall('#(.*?)$', url)
        dert = '"streamIds".*?shared_group_'+idd[0]+'.*?content":(.*?)description'
        request = urllib2.Request(url, None, {'User-agent': 'Mozilla/5.0 (Windows; U; Windows NT 5.1; en-GB; rv:1.9.0.3) Gecko/2008092417 Firefox/3.0.3','Connection': 'Close'})
        response = urllib2.urlopen(request)
        html = response.read()
        bundlelinks = re.findall(dert,html)[0]
        links=re.findall('https(.*?)"', bundlelinks)
        qualitylist = []
        videolist = []
        pre_link='https'
        for link_part in links:                       
                if link_part.find("=m18") > -1:
                        videolist.append(pre_link + link_part)
                        qualitylist.append("360p")
                if link_part.find("=m22") > -1:
                        videolist.append(pre_link + link_part)
                        qualitylist.append("720p")
                if link_part.find("=m37") > -1:
                        videolist.append(pre_link + link_part)
                        qualitylist.append("1080p")
        dialog = xbmcgui.Dialog()
        ret = dialog.select('kalite secin...',qualitylist)
